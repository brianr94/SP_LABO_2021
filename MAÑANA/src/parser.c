 ...
 *
 */
int parser_servicioFromText(FILE* pFile , LinkedList* pArrayList)
{
	e* servicio=NULL;
	char id[50];
	char descripcion[50];
	...
	if(pFile != NULL && pArrayList != NULL)
	{
		...
		do
		{
			...
			if(cantidadLineas==6)
			{
				servicio=e_newParametros(id, descripcion, tipo, precio, cantidad, total);

				if(servicio != NULL)
				{
					ll_add(pArrayList, servicio);
					retorno=0;
				}
			}
			else
